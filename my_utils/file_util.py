...
def _check_niput_file(filename:str=''):
    if filename.find(os.sep)>=0:
        just_dir_of_file(filename, create_floder=True)

def write_csv_row(file_name:str, context=[], 
                  create_dir=False, model = 'a', 
                  encoding='utf-8', just=False):
    """write csv row
    ...
    _check_niput_file(file_name)
    with open(file_name, model, encoding=encoding) as f:
        csv_writer = csv.writer(f)
        for con in context:
            csv_writer.writerow(con)

def read_josn(josn_file:str, encoding='utf-8'):
    # 打开josn文件
    with open(josn_file, 'r', encoding=encoding) as file:
