...
def my_print(*args,rc=True,sep=' ',if_last=False,color = ''):
    if color != "":
        ...
        for i in range(len(args)-1):
            p.append(com_str(args[i],rc,' ',last=if_last))
        print(''.join(p))
...
def just_dir_of_file(file_path:str, create_floder:bool=True, info=True):
    """_summary_
    ...
    return just_ff(_dir, create_floder = create_floder,info=info)
