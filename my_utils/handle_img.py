...
def D1toD3(img):
    ret = np.zeros((img.shape[0], img.shape[1], 3), dtype = img.dtype)
    ret[...,0] = img
    ret[...,1] = img
    ret[...,2] = img
    return ret
...
def cv2_writer(file_path, img, *,color='BGR', create_dir=False):
    if create_dir:
        just_dir_of_file(file_path)
...
def concat_img(images, 
    ...
    ret = make_grid(temp_images, nrow=math.ceil(len(temp_images)/nrow), 
