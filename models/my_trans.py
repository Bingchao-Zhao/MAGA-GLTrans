...
class COMA(nn.Module):
    def __init__(self, n_classes, in_feat=1024):
        super(COMA, self).__init__()
        self.local_att = LocalAtt(dim=512)
        self.global_att = GloalAtt(dim=512)
        self._fc1 = nn.Sequential(nn.Linear(in_feat, 512), nn.ReLU())
    ...
    def forward(self, **kwargs):
        ...
        # ---->Translayer x2
        h = self.layer2(local_att + global_att) #[B, N, 512]
        h = self.norm(h)
        h = h[:,0]
