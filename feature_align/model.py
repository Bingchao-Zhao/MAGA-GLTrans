...
model_urls = {
    ...
    'resnet50': 'weight/resnet50-19c8e357.pth',
...
class AlignNetwork(nn.Module):
    def __init__(self, hight_size=256, low_size=64, pretrained=True):
        super(AlignNetwork, self).__init__()
        self.h_model = resnet50_baseline(pretrained=pretrained)
        self.hight_size = hight_size
        for param in self.h_model.named_parameters():
            param[1].requires_grad = False
        self.low_size = low_size
        self.l_model = resnet50_baseline(pretrained=pretrained, mod='align')
        self.rate = [1,5,10,200]
        print(f"Input low size: {low_size}")
    def preprocess(self, h_img):
        # input_size = [h_img.shape[-2], h_img.shape[-1]]
        l_img = F.interpolate(h_img, size=(self.low_size, self.low_size))
        l_img = F.interpolate(l_img, size=[256, 256])
        return l_img
    def up_sample(self, l_img):
        return F.interpolate(l_img, size=(self.hight_size, self.hight_size))
    ...
    def caculate_loss_same_resolution(self, h_feature, l_feature, loss_fn):
        ...
        for i in range(len(h_feature)):
            ...
            total_loss += loss_fn[i](h_f, l_f)*self.rate[i]
    ...
    def forward(self, img, loss_fn=None, mod='infer'):
        if mod=='train':
            l_img = self.preprocess(img)
            h_feature = self.h_model(img)
            ...
            fake_h_img = self.up_sample(img)
            return self.l_model(fake_h_img)
        else:
            print(f"ERROR mod :'{mod}'")
